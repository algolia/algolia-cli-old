...
- `<algoliaApiKey>` | Required
- `<algoliaIndexName>` | Required
- `<outputPath>` | Required | Path to an existing local directory where output files will be saved (filenames are autogenerated).
...
```shell
algolia transferindex -a <sourceAlgoliaAppId> -k <sourceAlgoliaApiKey> -n <sourceAlgoliaIndexName> -d <destinationAlgoliaAppId> -y <destinationAlgoliaApiKey> -i <destinationIndexName>
```
...
- `<outputPath>` | Required | Path to an existing local directory where output files will be saved (saved output filenames will match corresponding source filenames).
